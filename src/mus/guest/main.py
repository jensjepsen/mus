...
class ProxyClient(mus.llm.types.LLM[Empty, str, None]):
    ...
    async def stream(
        self, **kwargs: t.Unpack[mus.llm.types.LLMClientStreamArgs[Empty, str]]
    ) -> t.AsyncGenerator[mus.llm.types.Delta, None]:
        result = wit_world.startstream(
            self.model_name, json.dumps(cattrs.unstructure(kwargs))
        )
...
def invoke_function(name: str, inputs: dict) -> t.Any:
    ...
    result = wit_world.runfunction(
        name, json.dumps(delta_converter.unstructure(inputs))
    )
    while delta := wit_world.pollstream(result):
        if delta == "[[STOP]]":
            break
...
def make_tool_proxies(
    tools: t.Dict[str, mus.llm.types.FunctionSchemaNoAnnotations],
) -> dict[str, t.Callable[..., t.Any]]:
    proxies = {}
    for name, func in tools.items():

        def make_proxy(
            f: mus.llm.types.FunctionSchemaNoAnnotations,
        ) -> ToolCallableWithCall:
            async def proxy(*args, **inputs):
                return invoke_function(f["name"], inputs)

            return ToolCallableWithCall(
                schema=mus.llm.types.FunctionSchema(**f, annotations=[]), function=proxy
            )
...
class WitWorld(wit_world.WitWorld):
    def run(
        self, code: str, inputs: str, llms: list[str], tools: str, functions: list[str]
    ) -> str:
        try:
            tools_list = delta_converter.structure(
                json.loads(tools),
                t.Dict[str, mus.llm.types.FunctionSchemaNoAnnotations],
            )
            deserialized_inputs = delta_converter.structure(
                json.loads(inputs), t.Dict[str, t.Any]
            )
                ...
                return json.dumps(
                    {
                        "status": "success",
                        "message": "Done",
                        "result": delta_converter.unstructure(globals["result"]),
                    }
                )
