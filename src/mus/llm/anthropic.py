...
def func_to_tool(func: FunctionSchemaNoAnnotations) -> at.ToolParam:
    p = at.ToolParam(
        name=func["name"], description=func["description"], input_schema=func["schema"]
    )
    return p
...
def str_to_text_block(s: str) -> at.TextBlockParam:
    return at.TextBlockParam({"type": "text", "text": s})
...
def tool_result_to_content(tool_result: ToolResult):
    if isinstance(tool_result.content.val, str):
        ...
        return [parse_content(c) for c in tool_result.content.val]
    else:
...
BlockType = t.Union[
    at.TextBlockParam,
    at.ImageBlockParam,
    at.ToolUseBlockParam,
    at.ToolResultBlockParam,
    at.ContentBlock,
]


def join_content(
    a: t.Union[t.List[t.Union[at.TextBlockParam, at.ImageBlockParam]], str],
    b: t.Union[t.List[t.Union[at.TextBlockParam, at.ImageBlockParam]], str],
):
    if isinstance(a, str):
...
class AnthropicLLM(
    ...
    def __init__(
        self,
        model: MODEL_TYPE,
        client: t.Optional[t.Union[AsyncAnthropicBedrock, AsyncAnthropic]] = None,
    ):
        if not client:
    ...
    async def stream(
        self, **kwargs: t.Unpack[LLMClientStreamArgs[STREAM_ARGS, MODEL_TYPE]]
    ):
        ...
        if functions := kwargs.get("functions", None):
            ...
            if function_choice := kwargs.get("function_choice", None):
                extra_kwargs["tool_choice"] = {"type": function_choice}
