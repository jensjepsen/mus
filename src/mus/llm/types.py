...
class LLM(ABC, t.Generic[STREAM_EXTRA_ARGS, MODEL_TYPE, CLIENT_TYPE]):
    ...
    @abstractmethod
    def stream(
        self, **kwargs: t.Unpack[LLMClientStreamArgs[STREAM_EXTRA_ARGS, MODEL_TYPE]]
...
class FallbackToolCallableType(t.Protocol):
    ...
    async def __call__(
        self, original_tool_name: str, original_input: t.Mapping[str, t.Any]
    ) -> ToolReturnValue | ToolValue: ...
...
def is_tool_return_value(val: t.Any) -> t.TypeGuard[ToolReturnValue]:
    return (
        ...
        or (isinstance(val, list) and all(is_tool_return_value(v) for v in val))
    )
