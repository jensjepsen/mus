...
def file_to_part(file: File):
    ...
    if type_ == "image":
        ...
        return genai_types.Part.from_bytes(
            data=base64.b64decode(file.content), mime_type=file.b64type
        )
    else:
...
def query_to_contents(query: Query):
    ...
    for q in query.val:
        if isinstance(q, Assistant):
            ...
            contents.append(
                genai_types.Content(
                    role="model", parts=[genai_types.Part.from_text(text=q.val)]
                )
            )
            current_role = "model"
        else:
    ...
    if current_parts:
        contents.append(
            genai_types.Content(role=current_role or "user", parts=current_parts)
        )
                        ...
                        genai_types.Content(
                            ...
                            parts=[
                                genai_types.Part(
                                    ...
                                    thought_signature=delta.metadata.get(
                                        "thought_signature"
                                    )
                                    if delta.metadata
                                    else None,
...
class GoogleGenAILLM(LLM[StreamArgs, MODEL_TYPE, genai.Client]):
    ...
    async def stream(
        ...
        if generate_content_config := (kwargs.get("kwargs", {}) or {}).get(
        ...
        if not kwargs.get("no_stream", False):
            try:
                response_stream = await self.client.aio.models.generate_content_stream(
                    model=str(self.model), contents=contents, config=config
                )
