...
def func_schema_to_tool(func_schema: FunctionSchemaNoAnnotations):
    return bt.ToolTypeDef(
        toolSpec=bt.ToolSpecificationTypeDef(
            ...
            inputSchema=bt.ToolInputSchemaTypeDef(json=func_schema["schema"]),
        )
    )
...
def file_to_image(file: File):
    try:
        ...
        raise ValueError(
            f"Invalid b64type: {file.b64type}, must be in format type/subtype"
        )
    if type_ != "image":
    ...
    return bt.ContentBlockTypeDef(
        image=bt.ImageBlockTypeDef(
            format=subtype,
            source=bt.ImageSourceTypeDef(bytes=base64.b64decode(file.content)),
        )
    )


def str_to_text_block(s: str):
    return bt.ContentBlockTypeDef(text=s)
...
def parse_tool_content(c: t.Union[str, File]):
    if isinstance(c, str):
        return bt.ToolResultContentBlockOutputTypeDef({"text": c})
    elif isinstance(c, File):
        img = file_to_image(c)
        if "image" in img:
            return bt.ToolResultContentBlockTypeDef(image=img["image"])
        else:
...
def has_reasoning_text(content: t.Union[bt.ContentBlockTypeDef, str]):
    if isinstance(content, str):
        ...
        "reasoningContent" in content and "reasoningText" in content["reasoningContent"]
    ):
        return True
    return False
...
def join_content(
    a: t.Union[t.List[bt.ContentBlockTypeDef], str],
    b: t.Union[t.List[bt.ContentBlockTypeDef], str],
):
    if isinstance(a, str):
    ...
    if "text" in a[-1] and "text" in b[0]:
        return a[:-1] + [bt.ContentBlockTypeDef({"text": a[-1]["text"] + b[0]["text"]})]
            ...
            if (
                ...
                messages.append(
                    bt.MessageTypeDef(
                        role="assistant",
                        content=[
                            bt.ContentBlockTypeDef(reasoningContent=reasoning_content)
                        ],
                    )
                )
...
class BedrockLLM(LLM[StreamArgs, MODEL_TYPE, BedrockRuntimeClient]):
    ...
    async def stream(
        ...
        extra_kwargs: dict[str, t.Any] = {**(kwargs.get("kwargs", None) or {})}
                    ...
                    async for event in response.get("stream"):
                            ...
                            if "toolUse" in delta:
                                ...
                                if current_function:
                                    ...
                                    current_function["input"] = (
                                        current_function.get("input", "") + tu["input"]
                                    )
                        ...
                        if "metadata" in event:
                            ...
                            usage = Usage(
                                ...
                                cache_read_input_tokens=metadata["usage"].get(
                                    "cacheReadInputTokens", 0
                                ),
                                cache_written_input_tokens=metadata["usage"].get(
