...
SandboxReturnValue = dict[str, t.Any]


class SandboxableCallable(t.Protocol):
    async def __call__(self) -> t.Optional[SandboxReturnValue]: ...


class SandboxReturnCallable(t.Protocol):
    async def __call__(self, **inputs: t.Optional[t.Any]) -> SandboxReturnValue: ...
...
def func_params_to_dataclass(func: t.Callable):
    ...
    fields = [
        (name, typ, dc.field(default=None))
...
def sandbox(**outer_kwargs: t.Unpack[SandboxSharedKwargs]):
    ...
    functions = {func.__name__: func for func in kwargs.get("functions", {})}

    tools = mus.functions.parse_tools(kwargs.get("tools", []))

    tool_map = {
        **{func.schema["name"]: func.function for func in tools},
    ...
    class Host(guest_imports.Host):
        ...
        def startstream(self, llm: str, kwargs: str) -> guest_types.Result[str, str]:
            ...
            unpickled_kwargs = delta_converter.structure(
                json.loads(kwargs), LLMClientStreamArgs[Empty, str]
            )
    ...
    @t.overload
    def wrapper(
        callable_or_code: str, **inputs: t.Optional[t.Any]
    ) -> t.Awaitable[SandboxReturnValue]: ...
    def wrapper(
        callable_or_code: t.Union[SandboxableCallable, str], **inputs: t.Optional[t.Any]
    ) -> t.Union[SandboxReturnCallable, t.Awaitable[SandboxReturnValue]]:
        async def inner(code: str, **inputs: t.Optional[t.Any]) -> SandboxReturnValue:
            if not code:
            ...
            if result.get("status") == "error":
                ...
                return delta_converter.structure(
                    result.get("result", {}) or {}, dict[str, t.Any]
                )
            else:
                raise RuntimeError(f"Invalid result from sandbox {result}")

        if callable(callable_or_code):
            inner_with_code = functools.partial(
                inner, callable_to_code(callable_or_code)
            )
