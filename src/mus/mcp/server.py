...
ResourceReadResponseContent = Union[
    ResourceReadResponseContentBlob, ResourceReadResponseContentText
]
...
class MCPServer:
    ...
    def __init__(
        self,
        read_line: Optional[ReadLineProtocol] = None,
        write_line: Optional[WriteLineProtocol] = None,
    ):
        self.methods = {}
        self.initialized = False
        self.set_stdio(
            read_line or self.default_read_line, write_line or self.default_write_line
        )
    ...
    def resource(
        ...
        def decorator(func: ResourceFunction):
            ...
            if not inspect.iscoroutinefunction(func):
                raise ValueError(
                    "Resource function must be a coroutine function (async def)"
                )

            inferred_name = name or getattr(func, "__name__", None)
            if not inferred_name:
                raise ValueError(
                    "Resource must have a name, either provided or inferred from the function name."
                )

            inferred_description = func.__doc__ or ""
            resource = ResourceRegistration(
                name=inferred_name, mimeType=mime_type, function=func
            )
            # Register the resource
            if (
                self.resource_uri_matcher.add_pattern(uri_template, resource)
                == "static"
            ):
    ...
    async def initialize(
        ...
        return {
            ...
            "serverInfo": {"name": "MCP Python Server", "version": "1.0.0"},
        }
    ...
    async def list_tools(self, **kwargs) -> ToolsListResponse:
        ...
        return {
            "tools": [
                tool_registration["tool"] for tool_registration in self.tools.values()
            ]
        }
    ...
    async def call_tool(
        ...
        if name in self.tools:
            ...
            if isinstance(result, str):
                ...
                result = [
                    ToolReturnValueText(type="text", text=r)
                    if isinstance(r, str)
                    else ToolReturnValueImage(
                        type="image", data=r.content, mimeType=r.b64type
                    )
            ...
            return {"content": [result] if not isinstance(result, list) else result}
                ...
                return ResourcesReadResponse(
                    contents=[
                        ResourceReadResponseContentText(
                            text=result, mimeType="text/plain", name=name, uri=uri
                        )
                    ]
                )
            elif isinstance(result, File):
    ...
    async def handle_request(self, request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "method" not in request or not isinstance(request["method"], str):
            return self._error_response(
                request_id,
                -32600,
                "Invalid Request: Method not specified or not a string",
            )
